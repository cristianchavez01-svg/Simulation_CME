...
fig.text(.5,.875,f'{T_GRAFICA_CINEMATICA_HORAS} horas de propagación',ha='center',fontsize=12,style='italic',color='#444')
...
fig.suptitle('Propagación conjunta: CME-1 y CME-2',fontsize=25.2,fontweight='normal',y=.99)
...
ax_d.yaxis.label.set_size(18.2)
...
ax_v.yaxis.label.set_size(18.2)
ax_v.xaxis.label.set_size(18.2)
...
for ax in (ax_d, ax_v):
    ...
    ax.set_xticks(minor_ticks, minor=True)
    ax.tick_params(axis='x', which='major', length=10, width=1.4, direction='in', labelsize=20)
    ax.tick_params(axis='x', which='minor', length=5, width=0.8, direction='in')
    ax.tick_params(axis='y', which='major', direction='in', right=True, labelsize=20)
    ax.tick_params(axis='y', which='minor', direction='in', right=True)
    ax.yaxis.set_ticks_position('both')
    ax.grid(which='minor', axis='x', alpha=0.15, ls='--')
    ax.grid(which='major', axis='x', alpha=0.35, ls='--')
