...
def velocidad(t):
    if t <= 0:
        return v02
    return v02 + quad(aceleracion, 0, t)[0]

def posicion(t):
    if t == 0:
        return x02
    tiempos_int = np.linspace(0, t, 100)
    vels = np.array([v02 + (quad(aceleracion, 0, ti)[0] if ti > 0 else 0)
                     for ti in tiempos_int])
    return x02 + np.trapz(vels, tiempos_int)
...
fig.suptitle('Cinemática de la CME - 3', fontsize=20, y=0.98)
...
plt.savefig("cinematica_original_CME3.pdf", dpi=300, bbox_inches='tight')
print("✓ Gráfica cinemática guardada: cinematica_original_CME3.pdf")
plt.show()

print("\nRESUMEN CINEMÁTICO:")
print(f"  Velocidad inicial:    {v02:.2f} km/s")
...
print(f"  Posición inicial:     {x02 / R_SOL_KM:.4f} R_sol")
print(f"  Posición final:       {posiciones[-1] / R_SOL_KM:.4f} R_sol")
print(f"  Distancia recorrida:  {(posiciones[-1] - x02) / R_SOL_KM:.4f} R_sol")

# ──────────────────────────────────────────────────────────────────────────────
# 2. PROPAGACIÓN EN COORDENADAS POLARES
# ──────────────────────────────────────────────────────────────────────────────
print("\n" + "="*80)
print("VISUALIZACIÓN POLAR: CME-3")
print("="*80)
...
fig.suptitle('Propagación de CME - 3', fontsize=18, fontweight='normal', y=0.99)
fig.text(0.5, 0.935, f'{T_HORAS} horas de propagación',
         ha='center', fontsize=13, style='italic', color='#444444')
...
plt.savefig("cme_evolucion_polar_CME3.pdf", dpi=300, bbox_inches='tight')
print("\n✓ Visualización guardada: cme_evolucion_polar_CME3.pdf")
plt.show()

print("\n" + "="*80)
print("✓ SIMULACIÓN CME-3 COMPLETADA")
print("="*80)
